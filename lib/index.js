...
const parser = require('posthtml')().constructor.parser
...
function executeScope (scope, locals, node) {
  ...
  return walk({ locals: scope }, trimTree(node.content))
}
...
function walk (opts, nodes) {
  ...
  // Save nodes.options and nodes.messages
  let reduced = []

  if (nodes.options) {
    reduced.options = nodes.options
  }

  if (nodes.messages) {
    reduced.messages = nodes.messages
  }
  ...
  return nodes.reduce((m, node, i) => {
    ...
    // if not, we have an object, so we need to run the tag, attributes and contents
    if (delimitersSettings.some(setting => setting.regexp.test(node.tag))) {
    ...
    if (node.attrs) {
      for (let key in node.attrs) {
        if (node.attrs.hasOwnProperty(key)) {
          if (delimitersSettings.some(setting => setting.regexp.test(key))) {
            let dumbAST = parser(`<i ${placeholders(key, ctx, delimitersSettings)}></i>`)
            Object.assign(node.attrs, dumbAST[0].attrs)
    ...
    if (node.tag === switches[0]) {
      ...
      Array.prototype.push.apply(m, walk(opts, trimTree(branch.content)))
    ...
    if (node.tag === loops[0]) {
      ...
      if (Array.isArray(target)) {
        for (let index = 0; index < target.length; index++) {
          loopedTree.push(executeLoop(keys, target[index], index, opts.locals, treeString))
        }
      } else {
        for (let key in target) {
          loopedTree.push(executeLoop(keys, target[key], key, opts.locals, treeString))
        }
      }
      ...
      return m.concat(trimTree(loopedTree.reduce((acc, val) => acc.concat(val), [])))
    }
    ...
    if (node.tag === scopes[0]) {
      ...
      let targetScope = node.attrs.with ? vm.runInContext(node.attrs.with, ctx) : {}
      ...
      const attrsKeys = Object.keys(node.attrs);

      // creates a copy of the keys that will be changed in targetScope by Object.assign
      const targetScopeBackup = makeLocalsBackup(attrsKeys, targetScope)
      Object.assign(targetScope, node.attrs)
      delete targetScope.with

      const targetScopeKeys = Object.keys(targetScope)

      // creates a copy of the keys that will be changed within the loop
      const localsBackup = makeLocalsBackup(targetScopeKeys, opts.locals)

      m.push(executeScope(targetScope, opts.locals, node))

      // returns the original keys values that was changed by Object.assign
      targetScope = revertBackupedLocals(attrsKeys, targetScope, targetScopeBackup)
      // returns the original keys values that was changed within the loop
      opts.locals = revertBackupedLocals(targetScopeKeys, opts.locals, localsBackup)
